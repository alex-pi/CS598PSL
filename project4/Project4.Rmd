...
A limitation of the naive approach is it assumes users rate movies using the same criteria (i.e. users who enjoy a movie or genre to an identical degree would give the same rating). It is not uncommon for users to have different interpretations even when using the same scoring system, and the differences introduce bias to the ratings for movies they have seen. Therefore, it's often beneficial to normalize the user's set of ratings to calibrate them to a common standard so the composition of all ratings is more consistent.
...
The first step is to normalize the data as users tend to rate movies according to their own methodology. Users with similar opinion will not necessarily assign the same numeric value as a rating. To reduce bias, each user's ratings are centered on the mean of their ratings set.(original): In the first step for this type of model, data is normalized. Similar to before, all users have their own tendencies to rate on average higher or lower. These averages are taken into consideration and means are set to zero across each user's ratings.
