...
Ratings data is loaded from ratings.dat and stored in a table with labeled columns as shown.
...
Movie data is loaded from movies.dat and stored in a table with columns 'MovieID', 'Title', 'Genres', and 'Year' with the year extracted from the movie title. The movie titles are re-encoded as some contain special characters which can create difficulties when comparing strings or other processing.
...
Next, we collect all movie ratings from the database, compute the average rating per movie, then use the average rating to select the 5 most popular movies in each genre.

```{r}
# get every rating for movies in the database
movies_ratings = left_join( movies, ratings, by="MovieID" )
movies_ratings = movies_ratings %>% 
  group_by( MovieID ) %>% 
  filter( n() >= 5 )

# compute mean average rating for each movie
...
# select 5 most popular movies in each genre
...
Users tend to rate movies according to their own methodology. Users with similar opinion will not necessarily assign the same numeric value as a rating. To reduce these deviant tendencies, we use the normalize function to center each user's ratings on the mean of their ratings set.

```{r}
# Step 1: Center both the train and test datasets
...
# Step 3: Top N most similar users
N = 20
...
top_20_similar_users_ratings_weighted = 
  weights * as( top_20_similar_users_ratings, "matrix" )
...
The estimate contains predictions for all movies from users in the neighborhood. Movies in the estimate already rated by the active user are set to NA, while the means of the ratings sets computed in the centering step are applied to restore the original ratings.

```{r}
# Step 5: Replace with NAs where already rated
tmp.y = as.vector( as( mytest, "matrix" ) )
ind.y = which( !is.na(tmp.y ) )
averaged_ratings_for_top_20_similar_users[ ind.y ] = NA

# apply user's ratings mean to restore original ratings
...
The 5 most popular movies from the model are selected and returned.
...
To reduce bias from idiosyncrasies in each user's rating methodology, the normalize function is used to center each user's ratings on the mean of their rating set.
...
The estimate contains ratings for all movie pairs in the neighborhood. The means computed in the centering step are applied to return ratings to their original range, then entries (movies) already rated by the active user are set to NA.

```{r}
# Step 5: Apply the mean for the active user
...
The 5 most popular movies are selected and returned.

```{r}
names( mypred ) = colnames( Rmat )
top_5_movies = tail( sort( mypred ), 5 )
top_5_movies = names( top_5_movies )
top_5_movies = strtoi( gsub( 'm', '', top_5_movies ) )
top_5_recommended_movies_IBCF = movies %>% filter( MovieID %in% top_5_movies )
top_5_recommended_movies_IBCF
```

**Question**: *Why do we encounter such a big discrepancy for IBCF, but not for UBCF?*

UBCF is a memory-based algorithm which computes predictions using the entirety of the ratings data. IBCF is a model-based algorithm which reduces the ratings data to a neighborhood (subset) of best candidates before computing predictions.

When using IBCF, the number of highly rated movies with identical similarity scores (S) can exceed the neighborhood size (N) causing many qualified movies to be discarded and produce prediction error if the best choices were among the discarded. The amount of error will vary depending on a few factors, such as size of N, how many movies in S reside at the neighborhood boundary, and the proportion of S that intersects the neighborhood. \
\
As neighborhood size decreases, the likeliness movies in S will be discarded increases. Movies in S residing on the boundary compete for the last few positions available in the neighborhood and affect prediction depending on which movies are chosen in the tie breaker process. For example, if the neighborhood size is 30 and movies in S occupy positions 28 through 36 in the ranked similarity scores, the quality of prediction will come down to choice of which of the 8 movies earn the last 3 positions (28, 29, 30). If S is large but intersection with the neighborhood is small, then prediction error will likely increase as many candidates are competing for few positions. If the intersection is large, then error is reduced.
