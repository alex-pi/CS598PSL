...
These next few models represent a more complex set of approaches to recommending movies to a user. This first of these models is a User-Based Content Filtering (UBCF) model. This model utilizes a nearest-neighbors approach between users to give recommendations.
...
In the first step for this type of model, data is normalized. Similar to before, all users have their own tendencies to rate on average higher or lower. These averages are taken into consideration and means are set to zero across each user's ratings.
...
Finally, the ratings are set to NA in our output vector for movies that the test user had already rated. Additionally, the test user's original ratings average is added back to the outputted vector of predicted ratings.
...
At this point, our UBCF model is complete and can be compared to recommenderlab's UBCF model with the same parameters. 
...
In order to properly suggest movies to the user, the top 5 rated movies from the model can be selected and outputted.
...
This second models is a Item-Based Content Filtering (IBCF) model. This model utilizes a nearest-neighbors approach between users to give recommendations, similar to the UBCF model.
...
Once again, data is normalized. Similar to before, all users have their own tendencies to rate on average higher or lower. These averages are taken into consideration and means are set to zero across each user's ratings.
...
Now that similarities between pairs of movies are computed, we retain only the highest scoring similarity values and eliminate the lower similarity scores. We retain only the similarities for the 30 most similarly rated movies.
...
for (i in 1:3706)
{
  ...
  tmp.x[is.na(tmp.x)] = 0
  top_30_by_similarity_indexes = tail(order(tmp.x, decreasing = FALSE), 30)
  sim_mat[i,top_30_by_similarity_indexes] = tmp.x[top_30_by_similarity_indexes]
}
```

The model then makes predictions about the test user's future ratings based on the user's existing ratings. This is done in a weighted manner by multiplying a vector of similarities by the existing ratings, summing up these products, and scaling them with respect to the utilized similarities.
...
To suggest movies to the test user, the top 5 rated movies from the model's output can be selected.
