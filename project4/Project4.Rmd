...
author: "Tyler Zender (tzender2), Matthew Lind (lind6), Alejandro Pimentel (ap41)"
date: "12/8/2022"
...
## System I: Recommendations based on movie genre
...
Next, we define 'popular' movies for each genre as the highest rated movies with at least 5 ratings. This '5 rating' lower-bound helps prevent recommending movies with high average ratings that were only recommended by a few individuals.
...
We test this approach with a test user's ratings.

```{r}
test_user = ratings[ which( ratings$UserID == 1 ), ]
test_user = left_join( test_user, movies, by="MovieID" )
test_user = test_user %>% 
  separate_rows( Genres, sep='\\|') %>% 
  group_by( Genres ) %>% 
  summarise_at( vars( Rating ), list( AvgRating = mean ) ) %>%
...
test_user_top_genre = test_user$Genres
```

The top genre for this user is `test_user_top_genre` for which we suggest the 5 most popular movies:

```{r}
top_movies_per_genre %>% filter( Genres == test_user_top_genre )
```

### Version 2: User-Normalized Ratings

A limitation of the naive approach is it assumes users rate movies using the same scale (i.e. users who enjoy a movie to an identical degree would give the same rating). It is not uncommon for users to score movies differently when controlling for this 'level of enjoyment'. The differences introduce bias to the ratings for movies they have seen. Therefore, it's often beneficial to normalize the user's set of ratings to calibrate them to a common standard so the composition of all ratings is more consistent.
...
Choosing a test user from the set, we evaluate the approach with the user's ratings.
...
`test_user_top_genre` is the top genre for this user, for which we suggest the 5 most popular movies.
...
The first step is to normalize the data as users tend to rate movies according to their own methodology. Users with similar opinions will not necessarily assign the same numeric value as a rating. To reduce bias, each user's ratings are centered by the mean of their ratings set.
...
The premise of this IBCF model is that the active user will prefer movies which are rated similar (by other users) to movies they like.

IBCF functions by first pre-computing the rating similarities across all possible pairs of movies as an M-by-M similarity matrix. For each movie, the top N most similar movies are selected and their similarities are stored in an M-by-M sparse matrix with only N*M positions filled. Future predictions for active users are based off these N most similar movies in this sparse matrix in conjunction with the active user's provided ratings. 

Because only similarities for the top N most similar movies of the original data are stored and used to provide predictions about new ratings, IBCF scales better than the memory-based algorithm of UBCF, but at the risk of potentially sacrificing quality when N is reduced substantially.
...
Data is normalized to reduce bias from idiosyncrasies in each user's rating methodology. Each user's ratings are centered by the mean of their rating set.
...
At this point, our IBCF model is complete and can be compared to recommenderlab's IBCF model with the same parameters.
