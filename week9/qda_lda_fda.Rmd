...
```{r}
sum(Ytest != Ytest.pred) / length(Ytest)
```
...
You can plot the projections on other directions. None of them produces meaningful results on the test data
...
Let’s try the following: use PCA to reduce the dimension from 256 to 100, and then apply LDA. What’s the accuracy?
